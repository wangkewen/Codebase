...
from scipy.spatial.distance import pdist,seuclidean,euclidean
from sklearn.metrics.pairwise import euclidean_distances
from sklearn.feature_extraction.text import TfidfTransformer
import scipy.cluster.hierarchy as shc

from matplotlib.mlab import PCA
from scipy.spatial.distance import minkowski,cosine,chebyshev,jaccard
...
def txtpca(label,m,nz,nn,X,u):
    ...
    if os.path.isfile('txtpca.mat')==True:
        ...
        print(scipy.io.whosmat("txtpca.mat"))
        index=txtpcaf['index']
    ...
    print(1.0*numpy.sum(eigenvalues[0,0:150])/numpy.sum(eigenvalues[0,:]))
    pylab.plot(numpy.arange(150),eigenvalues[0,0:150])
    pylab.show()
    if os.path.isfile('rX.mat'):
        ...
        rX=multiply(eigenvectors[:,0:150].T,xTx-uTx)
        print("rX",rX.shape)
...
######################################################            
'embeding'
def embed(label,m,nn,X,Xstd,u):
    print(X.shape)
    jacdis=numpy.zeros((m,m))
    for i in range(0,m-1):
        for j in range(i+1,m):
            #eudis[i,j]=minkowski(X[:,i],X[:,j],2)
            #l1dis[i,j]=minkowski(X[:,i],X[:,j],1)
            #cosdis[i,j]=cosine(X[:,i],X[:,j])
            #chedis[i,j]=chebyshev(X[:,i],X[:,j])
            jacdis[i,j]=jaccard(X[:,i],X[:,j])
            #use X not tfidfX
            jacdis[j,i]=jacdis[i,j]
        print(i)
    print(jacdis.shape)
    if not os.path.isfile('jacdis.mat'):
        scipy.io.savemat('jacdis.mat',{'dis':jacdis})
    else:
        jacdis=scipy.io.loadmat('jacdis.mat')['dis']
    print(jacdis)
